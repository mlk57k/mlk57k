...
export function AuthModal({ onClose }: AuthModalProps) {
  ...
  const completeUrl = "/auth/complete?next=/scan";
  ...
  const handleGoogle = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${origin}/auth/callback?next=${encodeURIComponent("/scan")}`,
      },
    });
    if (error) {
  ...
  return (
    ...
    >
      <div
        className="relative rounded-3xl bg-white shadow-2xl max-w-sm w-full p-8 mx-4"
        onPointerDown={(e) => e.stopPropagation()}
      >
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full text-stone-400 hover:bg-stone-100 hover:text-stone-600 transition-colors"
          aria-label="Fermer"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-stone-400">Gratuit · Résultats en 10 secondes</p>
        </div>
